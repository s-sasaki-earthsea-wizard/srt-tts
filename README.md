...
| `--no-consistency-check` | - | 短縮テキストの意味一貫性チェックを無効化 |
...
[言語バリデーション] 出力言語が不一致なら再生成（最大3回リトライ）
    ↓
[gTTS] 事前見積もり → 時間超過なら最大4回まで短縮を試行（意味一貫性チェック付き）
    ↓
[ElevenLabs TTS] 音声合成
    ↓
[速度調整] 時間超過なら最大2回まで短縮を試行（意味一貫性チェック付き） → それでも超過なら速度調整
    ↓
[音声結合] 最終MP3ファイル生成
```
