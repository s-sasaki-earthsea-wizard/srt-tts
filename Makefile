...
dub: ## 日本語SRTから吹き替え用SRTを生成 (SRT=<path> LANG=<code> [ARGS=...])
ifndef SRT
	$(error SRT is required. Usage: make dub SRT=srt/example-jp.srt LANG=en)
endif
ifndef LANG
	$(error LANG is required. Usage: make dub SRT=srt/example-jp.srt LANG=en)
endif
